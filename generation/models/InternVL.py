...
def get_model_and_tokenizer(
    path: str = 'OpenGVLab/InternVL2_5-4B'
):
...
def get_generation_config(tokenizer):
    return dict(max_new_tokens=1024, do_sample=False, pad_token_id=tokenizer.eos_token_id)
...
def load_videos(video_paths, bound=None, input_size=448, max_num=1, num_segments=32):
    pixel_values_list, num_patches_list = zip(*[
        load_video(video_path, bound, input_size, max_num, num_segments)
        for video_path in video_paths
    ])
    pixel_values_list = torch.cat(pixel_values_list).to(torch.bfloat16).cuda()
    num_patches_list = [item for sublist in num_patches_list for item in sublist]
    video_prefix = ''.join([f'Frame{i+1}: <image>\n' for i in range(len(num_patches_list))])
    return pixel_values_list, num_patches_list, video_prefix
...
def generate(
    ...
    pixel_values, num_patches_list, video_prefix = load_videos(
        [video_path.format(video_id=video['video_id']) for video in videos],
        num_segments=max_frames, max_num=1
    ) if len(videos) else (None, None, '')
